...
if exists("php_parent_error_open")
  syn region  phpComment  start="/\*" end="\*/" contained contains=phpTodo,phpDocTags,phpDocSeeTag,@Spell
else
  syn region  phpComment  start="/\*" end="\*/" contained contains=phpTodo, phpDocTags,phpDocSeeTag,@Spell extend
endif
if version >= 600
  syn match phpComment  "#.\{-}\(?>\|$\)\@="  contained contains=phpTodo,phpDocTags,phpDocSeeTag,@Spell
  syn match phpComment  "//.\{-}\(?>\|$\)\@=" contained contains=phpTodo,phpDocTags,phpDocSeeTag,@Spell
else
  syn match phpComment  "#.\{-}$" contained contains=phpTodo,phpDocTags,phpDocSeeTag,@Spell
  syn match phpComment  "#.\{-}?>"me=e-2  contained contains=phpTodo,phpDocTags,phpDocSeeTag,@Spell
  syn match phpComment  "//.\{-}$"  contained contains=phpTodo,phpDocTags,phpDocSeeTag,@Spell
  syn match phpComment  "//.\{-}?>"me=e-2 contained contains=phpTodo,phpDocTags,phpDocSeeTag,@Spell
endif
...
" PhpDoc
syntax case ignore
syn region phpDocTags    contained start="{@\(link\|linkplain\|inherit[Dd]oc\|doc[rR]oot\|value\)" end="}"
syn match  phpDocTags    contained "@\(param\|exception\|throws\|since\|return\)\s\+\S\+" contains=phpDocParam
syn match  phpDocParam   contained "\s\S\+"
syn match  phpDocTags    contained "@\(version\|author\|deprecated\|see\|test\|purpose\)\s\+.\+$" contains=phpDocParamLong
syn match  phpDocParamLong contained "\s\+.\+$"
syntax case match
...
if version >= 508 || !exists("did_php_syn_inits")
  ...
  HiLink   phpTodo  Todo
  HiLink   phpDoc  Keyword
  HiLink   phpDocTags  Keyword
  HiLink   phpDocParam Type
  HiLink   phpDocParamLong SpecialComment
  HiLink   phpDocSeeTag Special
